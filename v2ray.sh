...
pre_install_docker_compose(){
    ...
    read -p "(image address (Default ephz3nt/v2ray_v3:go):" docker_addresss
    [ -z "${docker_addresss}" ] && docker_addresss="ephz3nt/v2ray_v3:go"
    echo
    echo "---------------------------"
    ...
    [ -z "${MUSUFFIX}" ] && MUSUFFIX="www.baidu.com"
    echo
    echo "---------------------------"
...
config_docker(){
    ...
    curl -L https://raw.githubusercontent.com/NGKSSR/uussr/master/Docker/V2ray/docker-compose.yml > docker-compose.yml
...
config_caddy_docker(){
    ...
    echo "install curl"
    install_dependencies
    curl -L https://raw.githubusercontent.com/NGKSSR/uussr/master/Docker/Caddy_V2ray/Caddyfile >  Caddyfile
    echo "Writing docker-compose.yml"
    curl -L https://raw.githubusercontent.com/NGKSSR/uussr/master/Docker/Caddy_V2ray/docker-compose.yml > docker-compose.yml
    sed -i "s|MUREGEX:.*|MUREGEX: \"${MUREGEX}\"|"  ./docker-compose.yml
    sed -i "s|MUSUFFIX:.*|MUSUFFIX: \"${MUSUFFIX}\"|"  ./docker-compose.yml
    sed -i "s|xxx/xxx:xxx|${docker_addresss}|"  ./docker-compose.yml
    sed -i "s|node_id:.*|node_id: ${ssrpanel_node_id}|"  ./docker-compose.yml
